...
The difference between micro-db and most json-based databases is, that most json-based databases serialze the whole data object before it can be stored into a file. micro-db appends data snapshots in key-value pairs directly into the file. This has the huge advantage of instant-persistency whithout a big workload, because only the stored object has to be serialized.
...
To use the full power of typescript you should use micro-db as a sql database. Allthough, if type safty isn't a matter for you, you can use micro-db also as a nosql database (by using typescripts Union Types and Type Guards your nosql database can also be typesafe).
...
micro-db has a janitor built in. The main task of the [`MicroDBJanitor`](#microdbjanitor) is to clean redundanten / outdated snapshots from your database file.
...
The [`MicroDBJanitor`](#microdbjanitor) is responsible for cleaning up the database file for redundent records. The [`MicroDBJanitor`](#microdbjanitor) is configured with a cronjob, which determineds when and how often the janitor has to run. The more traffic or changes your database has, the more often the janitor should run to prevent huge overhead in file size.
...
To implement this in your code, you can extend the [`MicroDBFacade`](#microdbfacade) class. The `MicroDBFacade` provides the same api as a [`MicroDBDriver`](#microdbdriver), but all methods are protected, which means they are inaccessible from outside of the class.
